...
If one function calls another in nested way, the link register lr storing return address along with the frame pointer fp must be saved on the stack, so that when the child function returns to the parent function, the parent function can reload its lr and fp from the stack.
