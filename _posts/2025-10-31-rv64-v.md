...
The V extension defines 32 registers: v0-v31. The length of a vector register is set by VLEN: a vector register can hold four 32-bit numbers when VLEN = 128, or eight of them when VLEN = 256. A vector register can hold either multiple integers, or multiple floating-point numbers.
...
The number of loops will reduce four fold. Any user of the `vmul` routine will remain the same, as `main.c`.

## The hardware design
ETH Zurich and Barcelona Supercomputing Centre are notably doing hardware designs of the V extension.
...
The latest designs with throughput of 80 32-bit numbers, already run at below 1 Watt [Vitruvius+, Barcelona Supercomputing Centre]. Compare that with ARM64 or Apple's M microprocessors which usually run at 5 Watts at load.
