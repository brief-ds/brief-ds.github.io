...
In ARM64 SVE, one vector instruction can only be carried out over one vector register, largely. In the RISC-V64 V extension, one may group multiple vector registers for one vector instruction to compute on.

The `m1` label in the `vsetvli` instruction specifies that one vector instruction is carried out on one vector register only.
...
But we can make one vector instruction work on more vector registers, and thus more 32-bit elements. Note in the comment,
...
If one vector instruction can work on more 32-bit elements, the loop will require fewer steps to fully process the input.

The current designs, several of which are cited in the references, would generally impose a fixed maximum width for concurrent data, as for ARM64 SVE. For example, if one vector register is 128-bit, and the maximum width for concurrent data is 256-bit, then two vector registers can go through the data lane at the same time. But if the programmer specified `m4` grouping four vector registers for one vector instruction, the hardware will process content in the first two vector registers, then that in the next two vector registers, sequentially.

It seems the hardware should be built with simple units: each unit can be adaptively turned on/off, to maximise the power of the V extension, imparted by its flexibility.
