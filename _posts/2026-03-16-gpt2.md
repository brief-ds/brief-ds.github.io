...
Before training, we set up the model symbolically: specify how the functions are chained together, so that output of one is the input of another. No actual values are passed into the model. The `token_id`, `pos_id`, `target_id` below are just a placeholder.
